...
installation() {
	...
	[ ! -z "${PROGRAM_DIR}" ] && [ -d "${PROGRAM_DIR}" ] && SW_INSTALL="${PROGRAM_DIR}/INSTALL" || SW_INSTALL="${BINDIR}/INSTALL"
	...
	do
		case $1 in
			...
			;;
			*)
				echo "This function must have the --to and (--this or --these) flags populated with an argument."
				echo "Giving up..."
				exit 1
			;;
		esac
